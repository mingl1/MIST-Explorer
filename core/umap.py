...
class UMAPHelper:
    ...
    def run_umap(self, n_components=20, pca_method="manual", variance_threshold=0.85,
                 n_neighbors=15, min_dist=0.1, n_epochs=200, random_state=42, 
                 knn=False, kmeans=False, resolutions=[1.0]):
        # Use pre-computed PCA, but subset to n_components
        self.resolutions = resolutions
    
        # --- Compute neighbors ---
        # if knn:
        sc.pp.neighbors(self.adata, transformer=AnnoyTransformer(n_trees=50, n_neighbors=n_neighbors), n_pcs=n_components)
        # else:
    ...
    def rank_genes(self, adata, top_n=10, figsize=(10, 7)):
        ...
        ax.scatter(adata.var.means, adata.var.variances_norm, 
                s=30, alpha=1.0, c='gray', label='All genes')
        ...
        ax.set_title(f'Mean vs Variance (Top {top_n} Highly Variable Genes Labeled)', fontsize=14)
        ax.legend()
    ...
    def plot_umap(self, color="leiden", size=20, figsize=(10, 8), ax=None, 
                  show_centers=True, show=True, **kwargs):
    ...
    def show_colored_segmentation(self, seg, key='leiden', ignore= [], use_plotly=True, figsize=(12, 12)):
        ...
        leiden_to_color = {leiden: to_rgb(color) 
        ...
        for cell_id, leiden in tqdm(zip(self.adata.obs['cell_id'], 
        ...
        if missing_cells:
            print(f"Warning: {len(missing_cells)} cell IDs in segmentation are not in dataframe")
            print(f"These cells will be colored black: {sorted(list(missing_cells))[:10]}..." 
                  if len(missing_cells) > 10 else f"These cells will be colored black: {sorted(missing_cells)}")
