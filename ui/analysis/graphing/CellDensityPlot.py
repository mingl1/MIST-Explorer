...
class CellDensityPlot(QMainWindow):
    def __init__(self, data, region):
        ...
        self.figure, self.ax = plt.subplots(
            2, 1, figsize=(10, 12), constrained_layout=True
        )
    ...
    def plot_cell_density(self, data, region, markers):
        ...
        for region_name, (x_min, y_min, x_max, y_max) in regions.items():
            ...
            for marker in markers:
                ...
                marker_positive_cells = len(
                    cells_in_region[cells_in_region[marker] > 2000]
                )
        ...
        ax[0].set_title(
            "Marker Density Percentage Across Regions", fontsize=16, fontweight="bold"
        )
        ...
        for i, marker in enumerate(markers):
            ax[1].bar(
                x + i * width,
                region_df[f"{marker} Positive Cells"],
                width=width,
                label=marker,
                color=colors[i],
            )
        ...
        ax[1].set_title(
            "Total Marker+ Cells Across Regions", fontsize=16, fontweight="bold"
        )
